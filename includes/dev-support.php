...
// === Безопасные замеры шагов add_to_cart с выводом в футер ===

// --- Глобальный ключ для хранилища чекпоинтов
if ( ! defined( 'WCAC_CP_KEY' ) ) {
    define( 'WCAC_CP_KEY', 'wc_add_cart_cp' );
}

// --- Хелперы (с защитой от повторного объявления)
if ( ! function_exists( 'wcac_is_add_to_cart_ajax' ) ) {
    function wcac_is_add_to_cart_ajax() : bool {
...
if ( ! function_exists( 'wcac_mark' ) ) {
    function wcac_mark( $name ) {
        if ( ! wcac_is_add_to_cart_ajax() ) {
            return;
        }
        $t = microtime( true );
        if ( ! isset( $GLOBALS[ WCAC_CP_KEY ] ) || ! is_array( $GLOBALS[ WCAC_CP_KEY ] ) ) {
            $GLOBALS[ WCAC_CP_KEY ] = [];
        }
        $GLOBALS[ WCAC_CP_KEY ][] = [ 'name' => (string) $name, 't' => $t ];
    }
}

if ( ! function_exists( 'wcac_render_table' ) ) {
    function wcac_render_table() {
        $cps = isset( $GLOBALS[ WCAC_CP_KEY ] ) && is_array( $GLOBALS[ WCAC_CP_KEY ] )
            ? $GLOBALS[ WCAC_CP_KEY ] : [];

        if ( empty( $cps ) ) {
            return '<div id="wc-ajax-metrics" style="position:fixed;right:12px;bottom:12px;z-index:9999;padding:8px 10px;border-radius:10px;background:#f2f2f2;font:13px/1.35 system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;">Нет данных замеров.</div>';
        }
        ...
        foreach ( $cps as $i => $cp ) {
            ...
            $rows[]  = '<tr><td>'.intval($i).'</td><td>'.esc_html($cp['name']).'</td><td style="text-align:right">'.$step_ms.'&nbsp;мс</td><td style="text-align:right">'.$cum_ms.'&nbsp;мс</td></tr>';
            $prev = $cp['t'];
        }

        $last = $cps[ count($cps) - 1 ];
        $total_ms = (int) round( ( $last['t'] - $t0 ) * 1000 );

        $html  = '<div id="wc-ajax-metrics" style="position:fixed;right:12px;bottom:12px;z-index:9999;max-width:420px;background:#fff;border:1px solid rgba(0,0,0,.1);box-shadow:0 6px 24px rgba(0,0,0,.12);border-radius:10px;font:13px/1.35 system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif">';
        $html .= '<div style="display:flex;align-items:center;gap:10px;padding:10px 12px;border-bottom:1px solid rgba(0,0,0,.06)"><strong>WC add_to_cart — таймлайны</strong><span style="margin-left:auto;opacity:.7">Σ '.$total_ms.' мс</span></div>';
        $html .= '<div style="max-height:260px;overflow:auto"><table style="width:100%;border-collapse:collapse"><thead><tr style="background:#fafafa"><th style="text-align:left;padding:6px 10px;width:38px">#</th><th style="text-align:left;padding:6px 10px">Шаг</th><th style="text-align:right;padding:6px 10px;width:90px">Δ, мс</th><th style="text-align:right;padding:6px 10px;width:90px">Σ, мс</th></tr></thead><tbody>';
        $html .= implode( '', $rows );
        $html .= '</tbody></table></div>';
        $html .= '<div style="padding:8px 10px;color:#666;border-top:1px solid rgba(0,0,0,.06)">Обновляется после успешного add_to_cart</div>';
        $html .= '</div>';
...
// --- Плейсхолдер в футере (только фронт, не админ)
...
// --- Чекпоинты (выполняются ТОЛЬКО для нужного ajax)
...
add_filter( 'woocommerce_add_to_cart_fragments', function( $fragments ){
    // Страховка: выполняем только в нужном ajax
    if ( ! wcac_is_add_to_cart_ajax() ) {
        return $fragments;
    }

    wcac_mark( 'before fragments' );
    // … другие фрагменты могут быть добавлены темой/плагинами …
    wcac_mark( 'after fragments' );

    // Вставляем наш html в футер
