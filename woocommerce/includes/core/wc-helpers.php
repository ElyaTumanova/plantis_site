...
/* Проверяем принадлежность к родительской категории */

  function plnt_is_cat_or_child( $cat_id ) {
    ...
    return is_product_category( $cat_id )
      || term_is_ancestor_of( $cat_id, $term_id, 'product_cat' );
  }


/* находим тип каталога */


function plnt_define_catalog_type() {
    ...
    if ( plnt_is_cat_or_child( $plants_cat_id ) ) {
        return 'plants';
    }

    if ( plnt_is_cat_or_child( $treez_cat_id ) ) {
        return 'treez';
    }

    if ( plnt_is_cat_or_child( $treez_poliv_cat_id ) ) {
        return 'treez-poliv';
    }

    if ( plnt_is_cat_or_child( $ukhod_cat_id ) ) {
        return 'ukhod';
    }

    if ( plnt_is_cat_or_child( $plants_treez_cat_id ) ) {
        return 'plants-treez';
    }

    if ( plnt_is_cat_or_child( $lechuza_cat_id ) ) {
        return 'lechuza';
    }

    if ( plnt_is_cat_or_child( $gorshki_cat_id ) ) {
        return 'gorshki';
    }
...
function plnt_get_catalog_type() {
    static $catalog_type = null;

    if ( $catalog_type !== null ) {
        return $catalog_type;
    }

    $catalog_type = plnt_define_catalog_type();
...
function plnt_get_parent_cat_id($product = null) {

    static $parent_cat_id = null;
...
function plnt_is_treez_product($product = null) {

    static $is_treez = null;
    ...
    $is_treez = $product
        ? check_is_treez( $product )
        : false;

    return $is_treez;
}

function plnt_is_lechuza_product($product = null) {

    static $is_lechuza = null;
...
function plnt_get_catalog_context() {
