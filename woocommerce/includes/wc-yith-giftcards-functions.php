...
function plantis_send_gift_cards_on_completed( $order_id ) {
    ...
    foreach ( $order->get_items() as $item_id => $item ) {
        ...
        $gift_card_post_id  = isset( $gift_card_post_ids[0] ) ? absint( $gift_card_post_ids[0] ) : 0;
...
add_action('woocommerce_email_footer', function($email) {
  ...
  <p style="margin: 0 0 10px;"><a style="color: inherit;font-weight:normal;text-decoration:none" href="tel:+79995527944">8 999 552-79-44</a></p>
...
function plnt_get_giftcard_designs_config() {
	...
	if ( null === $config ) {
		$config = require get_template_directory() . '/assets/data/gift-card-designs.php';
	}
