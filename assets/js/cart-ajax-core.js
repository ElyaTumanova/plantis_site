...
// jQuery(function($){
// 	$( document ).on( 'plnt:cart-fragments-updated', function(){
// 		console.log('hi plnt:cart-fragments-updated');
//     productSliders.destroyAll();
//     productSliders.init();
// 	});
// })
  ...
  return fetch(woocommerce_params.ajax_url, {
    ...
    .then((data) => {
      ...
      document.dispatchEvent(
        new CustomEvent('plnt:cart-updated')
      );
...
/* CART FRAGMENTS RELOAD SYNC */
  async function plntUpdateCartWishlist() {
    ...
    try {
      ...
      if (miniCartHtml) {
        const miniCart = jQuery(miniCartHtml).get(0)

        if (miniCart) {
          updateCatalogButtons(miniCart)
        }
      }
  ...
  //HELPERS
  function updateCatalogButtons(miniCartRoot) {
  ...
  function updateWishCount(count) {
    ...
    document
      .querySelectorAll(
        '.header__main .header__wishlist .header__actions-count, ' +
        '.header__nav .header__wishlist .header__actions-count'
      )
      .forEach((element) => {
